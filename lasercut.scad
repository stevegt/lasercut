...
function start_x(edge, points) = points[edge][0];
function start_y(edge, points) = points[edge][1];
function end_x(edge, points) = points[edge+1][0];
function end_y(edge, points) = points[edge+1][1];

function adjacent(edge, points) = end_x(edge, points) - start_x(edge, points);
function opposite(edge, points) = end_y(edge, points) - start_y(edge, points);
function angle(edge, points) = atan2(opposite(edge, points), adjacent(edge, points));
function length(edge, points) = pow(pow(adjacent(edge, points), 2) + pow(opposite(edge, points), 2), .5);
...
)
{
    ...
    { 
        union() 
        {
            ...
            {
                simpleTab(simple_tabs[t][0], simple_tabs[t][1], simple_tabs[t][2], simple_tabs[t][3], 
                    thickness, points);
            }
            for (t = [0:1:len(captive_nuts)-1]) 
            {
                captiveNutTab(captive_nuts[t][0], captive_nuts[t][1], captive_nuts[t][2], thickness, points);
            }    
            ...
            {
                fingerJoint(finger_joints[t][0], finger_joints[t][1], finger_joints[t][2], thickness, points);
            }    
            for (t = [0:1:len(bumpy_finger_joints)-1]) 
            {
                fingerJoint(bumpy_finger_joints[t][0], bumpy_finger_joints[t][1], bumpy_finger_joints[t][2], thickness, points, bumps=true);
            }
            ...
            {
                clipTab(clips[t][0], clips[t][1], thickness, points);
            }  
        } // end union

        for (t = [0:1:len(simple_tab_holes)-1]) 
        {
            simpleTabHole(simple_tab_holes[t][0], simple_tab_holes[t][1], simple_tab_holes[t][2], 
                simple_tab_holes[t][3], simple_tab_holes[t][4], points);
        }
        for (t = [0:1:len(captive_nuts)-1]) 
        {
            captiveNutBoltHole(captive_nuts[t][0], captive_nuts[t][1], captive_nuts[t][2], thickness, points);
        }
        for (t = [0:1:len(captive_nut_holes)-1]) 
        {
            captiveNutHole(captive_nut_holes[t][0], captive_nut_holes[t][1], captive_nut_holes[t][2], captive_nut_holes[t][3], thickness, points);
        }    
        ...
        {
            twistConnect(twist_connect[t][0], twist_connect[t][1], thickness, points);
        }    
        for (t = [0:1:len(clips)-1]) 
        {
            clipInner(clips[t][0], clips[t][1], thickness, points);
        }    
        for (t = [0:1:len(clip_holes)-1]) 
        {
            clipHole(clip_holes[t][0], clip_holes[t][1], clip_holes[t][2], clip_holes[t][3], thickness, points);
        }    
...
module simpleTab(edge, offset, width, length, thickness, points)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
    edge_length = length(edge, points);

    translate([start_x,start_y,0]) rotate([0,0,angle]) 
        translate([offset*edge_length-width/2,-length,0]) cube([width, length, thickness]); 
}

module simpleTabHole(edge, offset, inset, width, tab_thickness, points)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
    ...
    translate([start_x,start_y,0]) rotate([0,0,angle]) 
        translate([offset*edge_length-width/2,inset,-tab_thickness*4]) cube([width, tab_thickness, tab_thickness*9]); 
}

module captiveNutBoltHole(edge, offset, nut_flat_width, thickness, points)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
    ...
    translate([start_x,start_y,0]) rotate([0,0,angle]) 
        translate([offset*edge_length,0,0])
    {
...
module fingerJoint(edge, offset, fingers, thickness, points, bumps = false, not_mill=true, milling_bit=0)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
    length = length(edge, points);
        
    fingers(start_x, start_y, angle, length, offset, fingers, thickness, bumps = bumps);

    // XXX deprecate milled fingers for now
    // XXX or better yet, always use milling code; kerf is laser beam width or milling bit size
    // fingers_mill(angle, start_up, fingers, thickness, range_min, range_max, t_x, t_y, bit=milling_bit);

}

module fingers(start_x, start_y, angle, length, offset, fingers, thickness, bumps = false)
{

    // The tweaks to y translate([0, -thickness,0]) ... thickness*2 rather than *1
    // Are to avoid edge cases and make the dxf export better.
    // All fun
    translate([start_x, start_y, 0]) rotate([0,0,angle]) translate([0, -thickness,0])
    {
        for ( p = [ 0 : 1 : fingers-1] )
		{
			...
			i = (length/fingers)*p;
            translate([i+(1-offset)*length/(fingers*2)-kerfMove,0,0]) 
            {
                cube([ length/(fingers*2)+kerfSize, thickness*2, thickness]);
                if(bumps == true)
                {
                    if (p < fingers-1) // XXX test bumps
                    {
                        translate([length/(fingers*2), thickness*.5, 0]) cylinder(h=thickness, r=thickness/10);
                    }
                }
            }
        }
...
module captiveNutTab(edge, offset, nut_flat_width, thickness, points)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
    ...
    translate([start_x,start_y,0]) rotate([0,0,angle]) translate([offset*edge_length,0,0])
    {
...
module captiveNutHole(edge, offset, inset, tab_thickness, thickness, points)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
    ...
    translate([start_x,start_y,0]) rotate([0,0,angle]) translate([offset*edge_length,inset,0]) 
    {
        translate([-thickness/2,0,-thickness]) cube([thickness, thickness, thickness*3]); 
        translate([-thickness/2+thickness*2,0,-thickness]) cube([thickness*3, thickness, thickness*3]); 
        translate([-thickness/2-thickness*4,0,-thickness]) cube([thickness*3, thickness, thickness*3]); 
    }
}

module twistHole(angle, center_x, center_y, length, thickness)
{
// http://msraynsford.blogspot.co.uk/2012/06/panel-joinery-10.html
    translate([center_x,center_y,0]) rotate([0,0,angle]) union()
    {
        cube([length, thickness, thickness*3], center=true); 
        difference()
        {
           // Need some trig,  radius is hypo of triangle
           // Other sides are thickness/2 and thickness*3
...
module twistConnect(edge, tab_size, thickness, points)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
    ...
    // translate([start_y,tab_size-thickness,-thickness]) rotate([0,0,angle]) union()
    // translate([start_x,start_y,0]) +tab_size,-thickness]) rotate([0,0,0]) union()
    // rotate([0,0,angle]) translate([0,tab_size,-thickness]) union()
    translate([start_x,start_y,0]) rotate([0,0,angle]) translate([0,tab_size,-thickness]) 
    {
        translate([-length/2-(thickness*3/2),0,0]) cube([length, thickness, thickness*3]); 
        translate([+length/2+(thickness*3/2),0,0]) cube([length, thickness, thickness*3]); 
    }

}

module clipInner(edge, offset, thickness, points)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
    ...
    translate([start_x,start_y,0]) rotate([0,0,angle]) translate([offset*edge_length,0,0]) rotate([0,0,180]) 
    {
...
module clipTab(edge, offset, thickness, points)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
...
module clipHole(edge, offset, inset, tab_thickness, thickness, points)
{
    start_x = start_x(edge, points);
    start_y = start_y(edge, points);
    angle = angle(edge, points);
    ...
    echo("angle", angle);
    echo("edge_length", edge_length);
    echo("offset", offset);
    echo("inset", inset);
    echo("tab_thickness,", tab_thickness);

    translate([start_x,start_y,0]) rotate([0,0,angle]) translate([offset*edge_length,inset*1.5,0]) 
    {
...
)
{
    ...
    {
        fj = [
            [ [2, 1, num_fingers], [0, 1, num_fingers],    ],
            [ [2, 0, num_fingers], [0, 1, num_fingers],    ],
            [ [2, 1, num_fingers], [0, 0, num_fingers],    ],
            [ [2, 1, num_fingers], [0, 1, num_fingers],    ],
        ];
    ...
    if (sides==5)
    {
        ...
        fj = [
            [ [2, 1, num_fingers], [0, 1, num_fingers], [3, 1, num_fingers],   ],
            [ [2, 0, num_fingers], [0, 1, num_fingers], [3, 1, num_fingers],   ],
            [ [2, 1, num_fingers], [0, 0, num_fingers], [3, 1, num_fingers],   ],
            [ [2, 1, num_fingers], [0, 1, num_fingers], [3, 1, num_fingers],   ],
            [ [2, 0, num_fingers], [0, 1, num_fingers], [3, 0, num_fingers], [1, 1, num_fingers]  ],
        ];
    ...
    if (sides==6)
    {
        ...
        fj = [
            [ [2, 1, num_fingers], [0, 1, num_fingers], [3, 1, num_fingers], [1, 0, num_fingers]  ],
            [ [2, 0, num_fingers], [0, 1, num_fingers], [3, 1, num_fingers], [1, 0, num_fingers]  ],
            [ [2, 1, num_fingers], [0, 0, num_fingers], [3, 1, num_fingers], [1, 0, num_fingers]  ],
            [ [2, 1, num_fingers], [0, 1, num_fingers], [3, 1, num_fingers], [1, 0, num_fingers]  ],
            [ [2, 0, num_fingers], [0, 1, num_fingers], [3, 0, num_fingers], [1, 1, num_fingers]  ],
            [ [2, 0, num_fingers], [0, 1, num_fingers], [3, 0, num_fingers], [1, 1, num_fingers]  ],
        ];
